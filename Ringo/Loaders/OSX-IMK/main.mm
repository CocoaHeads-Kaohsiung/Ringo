...
void processCmd(id ovService, int argc, char *argv[]) {
...
NSArray* processBundleFiles() {
  ...
  NSArray *loadPaths = [NSArray arrayWithObjects:modulePath, nil];
	...
	if ([mgr fileExistsAtPath:oldSearchPlist]) {
      ...
      NSError *error;
      [mgr moveItemAtPath:oldSearchPlist toPath:oneKeyPlist error:&error];
        //			[mgr movePath:oldSearchPlist toPath:oneKeyPlist handler:nil];
		} else {
      NSError *error;
      [mgr removeItemAtPath:oldSearchPlist error:&error];
...
int main(int argc, char *argv[]) {
  ...
  if (argc > 1) {
    ...
    if (ovService) {
      processCmd(ovService, argc, argv);
      return 0;
    }
  }
  ...
  [NSThread detachNewThreadSelector:@selector(start:) toTarget:ovl withObject:processBundleFiles()];
